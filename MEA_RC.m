...
stimNodes = num(:,5); % index of stimulation node
...
trialLength = 50000e-3*fs; % in frames -- length of trial
...
for n = 1:length(samples)
    ...
    if strcmp(stateVar, "spike_rates")
        ...
        mergedSpikeTimesFrames = cellfun(@(a) a*fs*1e-3, mergedSpikeTimes, 'UniformOutput', false); % convert from ms to frames
...
%     stimNode = stimElecIdx(2,:);
    ...
    t = plotPSTH(spikeTimes0, spikeTimes1, binsize, fs, trialsN, ISI, pairName{n}, baselineSpikeRate, xlim);
    cd .\network_response\PTSH
    saveas(t,pairName{n},'png')
    cd(homeDir)
    close all
